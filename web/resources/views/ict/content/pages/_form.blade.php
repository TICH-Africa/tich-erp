@props(['page' => null, 'statuses' => [], 'uploadUrl' => null])

@php
    $bodyValue = old('body', $page->body ?? '');
@endphp
...
<div class="tich-blog-compose__meta tich-card">
    <div class="tich-form-grid tich-form-grid--2">
        <div class="tich-form-group" style="grid-column:1/-1;">
            <label class="tich-label" for="page_title">Title</label>
            <input id="page_title" type="text" name="title" class="tich-input" value="{{ old('title', $page->title ?? '') }}" required maxlength="300">
        </div>
        <div class="tich-form-group">
            <label class="tich-label" for="page_status">Status</label>
            <select id="page_status" name="status" class="tich-input" required>
        ...
        <div class="tich-form-group">
            <label class="tich-label">Public path</label>
            <p class="tich-text" style="padding-top:0.55rem;"><code>/{{ $page->slug ?? '' }}</code></p>
        </div>
        <div class="tich-form-group">
            <label class="tich-label" for="page_seo_title">SEO title</label>
            <input id="page_seo_title" type="text" name="seo_meta_title" class="tich-input" maxlength="300" value="{{ old('seo_meta_title', $page->seo_meta_title ?? '') }}" placeholder="Optional — defaults to page title">
        </div>
        <div class="tich-form-group">
            <label class="tich-label" for="page_seo_description">SEO description</label>
            <textarea id="page_seo_description" name="seo_meta_description" class="tich-input" rows="2" maxlength="500" placeholder="Optional — shown in search results">{{ old('seo_meta_description', $page->seo_meta_description ?? '') }}</textarea>
        </div>
    </div>
</div>

<div class="tich-form-group tich-mt-6">
    <label class="tich-label" for="page_body">Page body</label>
    <div
        class="tich-cms-editor"
        data-cms-editor
        data-upload-url="{{ $uploadUrl }}"
        data-input-id="page_body"
    >
        ...
        <textarea id="page_body" name="body" class="tich-cms-hidden-input" required>{{ $bodyValue }}</textarea>
