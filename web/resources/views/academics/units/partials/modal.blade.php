<div class="tich-modal" id="{{ $modalId }}" aria-hidden="true" role="dialog" aria-modal="true">
    ...
    <div class="tich-modal__dialog">
        ...
        <form method="POST" action="{{ $unit ? route('departments.academics.units.update', array_merge($hub, ['unit' => $unit->id])) : route('departments.academics.units.store', $hub) }}" class="tich-modal__body" data-uf="skip">
            @csrf
            @if ($unit)
                @method('PUT')
            @endif
            ...
            @if (empty($defaultDepartmentId))
                <div class="uf-field">
                    ...
                    <select name="department_id" required @disabled($unit)>
            ...
            <div class="uf-form-grid-2">
                <div class="uf-field">
                    ...
                    <input type="text" name="unit_code" value="{{ old('unit_code', $unit?->unit_code) }}" required maxlength="30" placeholder="e.g. HSC 101">
                </div>
                <div class="uf-field">
                    <label>Contact hours</label>
                    <input type="number" name="contact_hours" min="0" value="{{ old('contact_hours', $unit?->contact_hours ?? 0) }}" placeholder="0">
                </div>
            </div>

            <div class="uf-field">
                <label>Unit name</label>
                <input type="text" name="unit_name" value="{{ old('unit_name', $unit?->unit_name) }}" required placeholder="e.g. Introduction to Anatomy">
            </div>

            <div class="uf-form-grid-2">
                <div class="uf-field">
                    <label>Total learning hours</label>
                    <input type="number" name="total_learning_hours" min="0" value="{{ old('total_learning_hours', $unit?->total_learning_hours ?? 0) }}" placeholder="0">
                </div>
                <div class="uf-field">
                    <label>Type</label>
            ...
            @if (! $unit && ! empty($selectedIntake) && $selectedIntake->status === 'draft' && ! empty($periods) && $periods->isNotEmpty())
                <div class="uf-field">
                    <label>Assign to semester after creating (optional)</label>
                    <select name="assign_semester">
            ...
            <div class="uf-field">
                ...
                <textarea name="description" rows="3" placeholder="Brief description of the unit...">{{ old('description', $unit?->description) }}</textarea>
            </div>
