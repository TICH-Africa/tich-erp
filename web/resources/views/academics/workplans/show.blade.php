...
@section('academics-content')
    <p class="tich-caption tich-mb-2">
        <a href="{{ route('departments.academics.workplans.index', $hubParams) }}" class="tich-link">← Semester workplans</a>
    </p>
    ...
    <div class="tich-grid tich-grid--2 tich-mt-6" style="align-items:start; gap:1.5rem;">
        ...
        @if ($canAct)
            @include('staff.workplans.partials.review-actions', [
                'approveRoute' => route('departments.academics.workplans.approve', array_merge($hubParams, ['workplan' => $workplan->id])),
                'rejectRoute' => route('departments.academics.workplans.reject', array_merge($hubParams, ['workplan' => $workplan->id])),
                'changesRoute' => route('departments.academics.workplans.request-changes', array_merge($hubParams, ['workplan' => $workplan->id])),
            ])
        @else
            <article class="tich-card">
                <h2 class="tich-h3">Review actions</h2>
                <p class="tich-text tich-mt-4">This workplan is not awaiting registrar action.</p>
            </article>
        @endif
    </div>
@endsection
