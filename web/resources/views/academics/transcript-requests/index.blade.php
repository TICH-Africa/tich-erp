...
@section('academics-content')
    ...
    <x-page-toolbar title="Transcript requests" meta="Official transcript issuance workflow">
        <x-slot:actions>
    ...
    <div class="tich-card tich-table-panel tich-mb-8">
        <form method="GET" action="{{ route('departments.academics.transcript-requests.index', $hub) }}" class="tich-flex-wrap" style="gap:0.75rem; align-items:end;">
            <div class="tich-form-group" style="margin:0;">
                <label for="status" class="tich-label">Status</label>
                <select id="status" name="status" class="tich-select">
                    @foreach (['pending' => 'Pending', 'processing' => 'Processing', 'issued' => 'Issued', 'rejected' => 'Rejected', 'all' => 'All'] as $value => $label)
                ...
                <tbody>
                    @forelse ($requests as $item)
                        <tr>
                            ...
                            <td class="tich-caption">{{ ucfirst($item->delivery_method) }}</td>
                            <td>{{ ucfirst($item->status) }}</td>
                            <td class="tich-caption">{{ $item->created_at?->format('d M Y H:i') }}</td>
                            <td>
                                <a href="{{ route('departments.academics.transcript-requests.show', array_merge($hub, ['transcriptRequest' => $item->id])) }}" class="tich-btn tich-btn-ghost">View</a>
                            </td>
                        </tr>
                    @empty
                        @include('partials.states.table-empty', ['colspan' => 5, 'title' => 'No transcript requests', 'icon' => 'inbox'])
                    @endforelse
                </tbody>
            </table>
        </div>
