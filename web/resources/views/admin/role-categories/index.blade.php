...
@section('admin-content')
    ...
    <div class="tich-card tich-table-panel">
        <table class="tich-admin-table tich-category-sortable">
            ...
            <tbody
                data-row-sortable
                data-sort-url="{{ route('admin.role-categories.reorder') }}"
                    ...
                    <tr data-sortable-row data-sort-id="{{ $categoryItem->id }}">
                        ...
                        <td style="white-space: nowrap;">
                            <button
                                ...
                                data-update-url="{{ route('admin.role-categories.update', $categoryItem) }}"
                            ...
                            @unless ($categoryItem->is_system)
                                <form method="POST" action="{{ route('admin.role-categories.destroy', $categoryItem) }}" style="display: inline;" onsubmit="return confirm('Delete category {{ $categoryItem->category_name }}?');">
                                    @csrf
                                    @method('DELETE')
    ...
    <div id="category-create-modal" class="tich-modal{{ $openCreateModal ? ' is-open' : '' }}" aria-hidden="{{ $openCreateModal ? 'false' : 'true' }}" role="dialog" aria-modal="true">
        ...
        <div class="tich-modal__dialog">
            ...
            <form method="POST" action="{{ route('admin.role-categories.store') }}" class="tich-modal__body">
                @csrf
    ...
    <div id="category-edit-modal" class="tich-modal{{ $openEditCategoryId ? ' is-open' : '' }}" aria-hidden="{{ $openEditCategoryId ? 'false' : 'true' }}" role="dialog" aria-modal="true">
        ...
        <div class="tich-modal__dialog">
            ...
            <form id="category-edit-form" method="POST" action="{{ $editCategory ? route('admin.role-categories.update', $editCategory) : '#' }}" class="tich-modal__body">
                @csrf
                @method('PUT')
