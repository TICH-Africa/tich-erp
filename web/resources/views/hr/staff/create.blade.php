        ...
        <form method="POST" action="{{ route('hr.staff.store') }}">
            ...
            <div class="tich-grid tich-grid--2 tich-mb-6">
                ...
                <div>
                    ...
                    <input type="text" id="first_name" name="first_name" value="{{ old('first_name') }}" required class="tich-input">
                </div>
                <div>
                    <label for="middle_name" class="tich-label">Middle Name</label>
                    <input type="text" id="middle_name" name="middle_name" value="{{ old('middle_name') }}" class="tich-input">
                </div>
                <div>
                    <label for="surname" class="tich-label">Surname *</label>
                    <input type="text" id="surname" name="surname" value="{{ old('surname') }}" required class="tich-input">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select id="employment_category" name="employment_category" required class="tich-input">
                        ...
                        <option value="permanent" {{ old('employment_category') == 'permanent' ? 'selected' : '' }}>Permanent</option>
                        <option value="contract" {{ old('employment_category') == 'contract' ? 'selected' : '' }}>Contract</option>
                        <option value="intern" {{ old('employment_category') == 'intern' ? 'selected' : '' }}>Intern</option>
                        <option value="visiting" {{ old('employment_category') == 'visiting' ? 'selected' : '' }}>Visiting</option>
                        <option value="casual" {{ old('employment_category') == 'casual' ? 'selected' : '' }}>Casual</option>
                    </select>
                </div>
                <div>
                    <label for="employment_start_date" class="tich-label">Start Date *</label>
                    <input type="date" id="employment_start_date" name="employment_start_date" value="{{ old('employment_start_date') }}" required class="tich-input">
                </div>
                <div>
                    <label for="gross_monthly_salary" class="tich-label">Gross Monthly Salary *</label>
                    <input type="number" step="0.01" id="gross_monthly_salary" name="gross_monthly_salary" value="{{ old('gross_monthly_salary') }}" required class="tich-input">
                </div>
            </div>

            <div class="tich-mt-6">
                <button type="submit" class="tich-btn tich-btn-primary">Create Staff Member</button>
                <a href="{{ route('hr.staff.index') }}" class="tich-btn tich-btn-ghost">Cancel</a>
            </div>
        </form>
    </article>
@endsection
