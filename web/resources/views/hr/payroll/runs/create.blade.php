...
@section('hr-content')
    <x-page-toolbar title="New payroll run" meta="Calculate and save a monthly payroll batch from current staff records">
        <x-slot:actions>
            <a href="{{ route('hr.payroll.runs.index') }}" class="tich-btn tich-btn-ghost">Back</a>
        </x-slot:actions>
    </x-page-toolbar>

    <article class="tich-card tich-mt-8">
        <form method="post" action="{{ route('hr.payroll.runs.store') }}" class="tich-form-grid tich-form-grid--2">
            @csrf
