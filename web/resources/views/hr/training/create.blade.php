            ...
            <div class="tich-grid tich-grid--2 tich-mb-6">
                <div>
                    ...
                    <select id="staff_ids" name="staff_ids[]" multiple class="tich-input" style="min-height: 10rem;">
                        ...
                        @foreach ($staff as $s)
                            <option value="{{ $s->id }}" {{ old('staff_ids') && in_array($s->id, old('staff_ids')) ? 'selected' : '' }}>
                ...
                <div>
                    <label class="tich-checkbox tich-mt-6">
                        <input type="checkbox" id="assign_all" name="assign_all" value="1" {{ old('assign_all') ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <select id="activity_type" name="activity_type" required class="tich-input">
                        ...
                        <option value="cpd" {{ old('activity_type') == 'cpd' ? 'selected' : '' }}>CPD</option>
                        <option value="study_leave" {{ old('activity_type') == 'study_leave' ? 'selected' : '' }}>Study Leave</option>
                        <option value="attachment" {{ old('activity_type') == 'attachment' ? 'selected' : '' }}>Attachment</option>
                        <option value="mentorship" {{ old('activity_type') == 'mentorship' ? 'selected' : '' }}>Mentorship</option>
                    </select>
                </div>
                <div>
                    <label for="activity_name" class="tich-label">Activity Name *</label>
                    <input type="text" id="activity_name" name="activity_name" value="{{ old('activity_name') }}" required class="tich-input">
                </div>
                <div>
                    <label for="organizer" class="tich-label">Organizer</label>
                    <input type="text" id="organizer" name="organizer" value="{{ old('organizer') }}" class="tich-input">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" id="hours_or_days" name="hours_or_days" value="{{ old('hours_or_days') }}" class="tich-input">
                </div>
                <div>
                    <label for="cpd_credits_earned" class="tich-label">CPD Credits</label>
                    <input type="number" step="0.01" id="cpd_credits_earned" name="cpd_credits_earned" value="{{ old('cpd_credits_earned') }}" class="tich-input">
                </div>
                <div>
                    <label for="location" class="tich-label">Location</label>
                    <input type="text" id="location" name="location" value="{{ old('location') }}" class="tich-input">
                </div>
                <div>
                    <label for="cost" class="tich-label">Cost (KES)</label>
                    <input type="number" step="0.01" id="cost" name="cost" value="{{ old('cost') }}" class="tich-input">
                </div>
                <div>
                    <label for="funded_by" class="tich-label">Funded By</label>
                    <select id="funded_by" name="funded_by" class="tich-input">
                        <option value="">Select</option>
                        <option value="institution" {{ old('funded_by') == 'institution' ? 'selected' : '' }}>Institution</option>
                        <option value="self" {{ old('funded_by') == 'self' ? 'selected' : '' }}>Self</option>
                        <option value="donor" {{ old('funded_by') == 'donor' ? 'selected' : '' }}>Donor</option>
                        <option value="sponsor" {{ old('funded_by') == 'sponsor' ? 'selected' : '' }}>Sponsor</option>
                    </select>
                </div>
                <div class="tich-flex--center">
                    <label class="tich-checkbox">
                        <input type="checkbox" id="is_external" name="is_external" value="1" {{ old('is_external') ? 'checked' : '' }}>
                        <span>External</span>
                    </label>
                </div>
                <div class="tich-flex--center">
                ...
                <div class="tich-grid--span-2">
                    ...
                    <textarea id="appraisal_relevance" name="appraisal_relevance" rows="2" class="tich-input">{{ old('appraisal_relevance') }}</textarea>
                </div>
            </div>

            <div class="tich-mt-6">
                <button type="submit" class="tich-btn tich-btn-primary">Save Training</button>
