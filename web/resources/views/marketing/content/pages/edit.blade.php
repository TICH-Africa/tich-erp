...
@section('department-content')
    <x-page-toolbar title="Edit {{ $page->title }}" meta="Public URL: /{{ $page->slug }}">
        <x-slot:actions>
            ...
            @endif
            <a href="{{ route('marketing.pages.index') }}" class="tich-btn tich-btn-secondary">Back to list</a>
        </x-slot:actions>
    </x-page-toolbar>

    <form method="POST" action="{{ route('marketing.pages.update', $page) }}" class="tich-blog-compose" data-blog-compose>
        @csrf
        @method('PUT')
        @include('marketing.content.pages._form', [
            'page' => $page,
            'statuses' => $statuses,
            'uploadUrl' => route('marketing.pages.upload-image'),
        ])
        <div class="tich-blog-compose__footer">
            <a href="{{ route('marketing.pages.index') }}" class="tich-btn tich-btn-secondary">Cancel</a>
