...
@section('department-content')
    ...
    <div class="tich-card tich-table-panel">
        <table class="tich-admin-table tich-site-settings-sortable">
            ...
            <tbody
                data-row-sortable
                data-sort-url="{{ route('marketing.about.reorder') }}"
                    ...
                    <tr data-sortable-row data-sort-id="{{ $block->id }}">
                        ...
                        <td style="display:flex;gap:0.35rem;">
                            <button
                                ...
                                data-update-url="{{ route('marketing.about.update', $block) }}"
                                data-id="{{ $block->id }}"
                            ...
                            <form method="POST" action="{{ route('marketing.about.destroy', $block) }}" onsubmit="return confirm('Delete this About section?');">
                                @csrf
                                @method('DELETE')
    ...
    <div id="about-create-modal" class="tich-modal{{ $openCreate ? ' is-open' : '' }}" aria-hidden="{{ $openCreate ? 'false' : 'true' }}">
        ...
        <div class="tich-modal__dialog">
            ...
            <form method="POST" action="{{ route('marketing.about.store') }}" enctype="multipart/form-data" class="tich-modal__body">
                @csrf
                @include('marketing.content.about._form', ['prefix' => 'create_'])
    ...
    <div id="about-edit-modal" class="tich-modal{{ $editBlock ? ' is-open' : '' }}" aria-hidden="{{ $editBlock ? 'false' : 'true' }}">
        ...
        <div class="tich-modal__dialog">
            ...
            <form id="about-edit-form" method="POST" action="{{ $editBlock ? route('marketing.about.update', $editBlock) : '#' }}" enctype="multipart/form-data" class="tich-modal__body">
                @csrf
                @method('PUT')
                <input type="hidden" name="edit_block_id" id="edit_block_id" value="{{ $editBlock?->id }}">
                @include('marketing.content.about._form', ['block' => $editBlock, 'prefix' => 'edit_'])
                <div class="tich-modal__footer">
