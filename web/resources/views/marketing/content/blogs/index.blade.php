...
@section('department-content')
    <x-page-toolbar title="Blog posts" meta="Homepage and public blog content">
        <x-slot:actions>
            <a href="{{ route('marketing.blogs.create') }}" class="tich-btn tich-btn-primary">Add post</a>
        </x-slot:actions>
    </x-page-toolbar>
            ...
            <tbody>
                @forelse ($posts as $post)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('marketing.blogs.edit', $post) }}" class="tich-link">{{ $post->title }}</a>
                        ...
                        <td style="display:flex;gap:0.35rem;">
                            <a href="{{ route('marketing.blogs.edit', $post) }}" class="tich-squircle-btn" title="Edit">
                            ...
                            <form method="POST" action="{{ route('marketing.blogs.destroy', $post) }}" onsubmit="return confirm('Delete this post?');">
                                @csrf
                                @method('DELETE')
