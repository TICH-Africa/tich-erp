...
@section('finance-content')
    <x-page-toolbar :title="$run->run_number" :meta="$run->periodLabel() . ' · ' . $run->staff_count . ' staff · ' . ucfirst($run->status)">
        <x-slot:actions>
            @if ($run->isEditable())
                <form method="post" action="{{ route('finance.employee.payroll.runs.recalculate', $run) }}" style="display:inline;">
                    @csrf
            ...
            @if ($run->canApprove())
                @can('finance.read')
                    <form method="post" action="{{ route('finance.employee.payroll.runs.approve', $run) }}" style="display:inline;">
                        @csrf
            ...
            @endif
            @if ($run->isEditable())
                <form method="post" action="{{ route('finance.employee.payroll.runs.cancel', $run) }}" style="display:inline;" onsubmit="return confirm('Cancel this draft payroll run?');">
                    @csrf
                    <button type="submit" class="tich-btn tich-btn-ghost">Cancel</button>
                </form>
            @endif
            <a href="{{ route('finance.employee.payroll.runs.index') }}" class="tich-btn tich-btn-ghost">All runs</a>
        </x-slot:actions>
    </x-page-toolbar>
    ...
    @if ($run->status !== \App\Models\PayrollRun::STATUS_DRAFT)
        <section class="tich-dept-panel tich-mt-8">
            ...
            <div class="tich-flex" style="gap:0.5rem; flex-wrap:wrap;">
                <a href="{{ route('finance.employee.payroll.runs.statutory.export', [$run, 'kra']) }}" class="tich-btn tich-btn-secondary">KRA (PAYE/WHT) CSV</a>
                <a href="{{ route('finance.employee.payroll.runs.statutory.export', [$run, 'nssf']) }}" class="tich-btn tich-btn-secondary">NSSF CSV</a>
                <a href="{{ route('finance.employee.payroll.runs.statutory.export', [$run, 'sha']) }}" class="tich-btn tich-btn-secondary">SHA/SHIF CSV</a>
            </div>
        </section>
    @endif
                    ...
                    @forelse ($run->items as $item)
                        <tr>
                            ...
                            <td style="white-space:nowrap;">
                                <a href="{{ route('finance.employee.payroll.runs.item.payslip', $item) }}" class="tich-btn tich-btn-ghost" target="_blank" rel="noopener">View</a>
                                <a href="{{ route('finance.employee.payroll.runs.item.payslip.pdf', $item) }}" class="tich-btn tich-btn-ghost">PDF</a>
                            </td>
                        </tr>
                    @empty
                        <tr>@include('partials.states.table-empty', ['colspan' => 7, 'title' => 'No staff with pay in this run.', 'icon' => 'inbox'])</tr>
                    @endforelse
                </tbody>
            </table>
        </div>
