@php
    ...
    $signStoreRoute = $signStoreRoute ?? 'finance.financial-policies.sign.store';
...
@section('title', 'Sign Financial Policy')

@section($moduleContext['content_section'])
    <x-page-toolbar
        title="Mandatory financial policy sign-off"
    ...
    <article class="tich-card tich-mt-6">
        <p class="tich-text">
            @if ($department)
                The Head of <strong>{{ $department->dept_name }}</strong> must view and digitally sign this financial policy before submitting annual budgets and departmental plans for this unit.
            @else
                Heads of Department must view and digitally sign this financial policy before submitting annual budgets and departmental plans.
            @endif
        </p>
    ...
    @if (($document['exists'] ?? false))
        <section class="tich-mt-6 finance-policy-sign__document">
            <div style="display:flex;flex-wrap:wrap;justify-content:space-between;gap:1rem;align-items:center;">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:0.5rem;">
                    <a
                        href="{{ route('finance.financial-policies.view', $policy) }}"
                        class="tich-btn tich-btn-secondary"
                        target="_blank"
                        rel="noopener"
                    >Open in new tab</a>
                    <a href="{{ route('finance.financial-policies.download', $policy) }}" class="tich-btn tich-btn-ghost">Download</a>
                </div>
            </div>
                    ...
                    <div class="doc-viewer__stage">
                        @if ($document['is_previewable'])
                            @if (str_starts_with($document['mime'], 'image/'))
                                <img
                                    src="{{ route('finance.financial-policies.view', $policy) }}"
                                    alt="{{ $policy->title }}"
                        ...
                        @else
                            <div class="doc-viewer__fallback">
                                ...
                                <a href="{{ route('finance.financial-policies.download', $policy) }}" class="tich-btn tich-btn-primary tich-mt-4">Download file</a>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <style>
            .finance-policy-sign__document .doc-viewer__panel {
                border: none;
                border-radius: 0;
                background: transparent;
            }
            .finance-policy-sign__document .doc-viewer__toolbar {
                border-bottom: none;
                padding-left: 0;
                padding-right: 0;
                background: transparent;
    ...
    @unless ($alreadySigned)
        <section class="tich-mt-6">
            ...
            <form method="POST" action="{{ route($signStoreRoute) }}" class="tich-form-stack tich-mt-4">
                ...
                <button type="submit" class="tich-btn tich-btn-primary">I have read and sign this financial policy</button>
            </form>
        </section>
    @endunless
